  ...
  try {
    ...
    return Promise.all(repos.map(async (repo: any) => {
      ...
      return {
        ...
        complexFunction: "Complexity analysis not available", // Placeholder
        duplicateArea: "Duplication analysis not available", // Placeholder
      };
    }));
  } catch (error) {
    console.error('Error fetching repositories:', error);
...
async function generateSuggestions(repo: any, metrics: any, token: string): Promise<string[]> {
  ...
  if (metrics.testCoverage < 70) {
    const untested = await getUntestedFiles(repo, token, contents);
  ...
  if (metrics.openIssuesAndPRs < 70) {
    const topIssue = await getTopIssue(repo, token);
    const oldestPR = await getOldestPR(repo, token);
    suggestions.push(`🔍 Address open issues and PRs in ${repo.name}:\n   - Oldest issue: "${topIssue}"\n   - Longest open PR: "${oldestPR}"`);
  }

  if (metrics.documentationQuality < 70) {
    const undocumented = await getUndocumentedFiles(repo, token, contents);
