...
async function fetchRepositoryContents(repo: any, token: string): Promise<any[]> {
  try {
    ...
    // If the 'main' branch doesn't exist, try 'master'
    try {
      const response = await axios.get(`https://api.github.com/repos/${repo.full_name}/git/trees/master?recursive=1`, {
        headers: { Authorization: `token ${token}` },
      });
      return response.data.tree || [];
    } catch (masterError) {
      console.error('Error fetching repository contents from master branch:', masterError);
      return [];
    }
  }
}
...
async function generatePersonalizedSuggestions(repo: any, token: string): Promise<string[]> {
  ...
  // Add more personalized checks and suggestions here
