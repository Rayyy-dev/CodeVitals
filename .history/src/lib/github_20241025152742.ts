...
export async function fetchUserRepositories(token: string): Promise<Repository[]> {
  try {
    ...
    console.log('Public repositories:', allRepos.filter(repo => repo.visibility === 'public').length);
    console.log('Private repositories:', allRepos.filter(repo => repo.visibility === 'private').length);
