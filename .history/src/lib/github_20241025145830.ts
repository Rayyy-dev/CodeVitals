...
async function fetchRepositoryContents(repo: any, token: string): Promise<any[]> {
  try {
    ...
    // If there's an error (including 409), return an empty array
    return [];
  }
}
...
async function generatePersonalizedSuggestions(repo: any, token: string): Promise<string[]> {
  ...
  if (contents.length === 0) {
    suggestions.push(`Unable to analyze ${repo.name} contents. Please check repository permissions and try again.`);
    return suggestions;
  }
