...
export async function fetchRepositories(token: string): Promise<Repository[]> {
  try {
    ...
    return processedRepos.filter((repo): repo is Repository => {
      return repo !== null && 'id' in repo && 'name' in repo && 'full_name' in repo &&
             'private' in repo && 'quality_score' in repo && 'metrics' in repo &&
             'suggestions' in repo && 'topIssue' in repo && 'oldestPR' in repo &&
             'complexFunction' in repo && 'duplicateArea' in repo;
    });
  } catch (error) {
    console.error('Error fetching repositories:', error);
    throw error;
