...
const FeatureCard = ({ icon: Icon, title, description }: { icon: React.ElementType; title: string; description: string }) => (
  <motion.div
    ...
    className="h-full bg-white border border-gray-200 overflow-hidden group rounded-2xl transform transition-all duration-300 hover:scale-105"
  >
    <div className="p-8 transition-colors duration-300 group-hover:bg-blue-50 h-full flex flex-col">
      <div className="flex items-center mb-4">
        <div className="bg-blue-100 p-3 rounded-full mr-4 transition-colors duration-300 group-hover:bg-blue-200">
          <Icon className="w-6 h-6 text-blue-600 transition-colors duration-300 group-hover:text-blue-700" />
        </div>
        <h3 className="text-xl font-semibold text-gray-900 transition-colors duration-300 group-hover:text-blue-700">{title}</h3>
      </div>
      <p className="text-gray-600 transition-colors duration-300 group-hover:text-blue-800 flex-grow">{description}</p>
    </div>
  </motion.div>
);
