...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const [repoData, issuesData, languagesData, commitsData, branchesData] = await Promise.all([
      ...
      octokit.issues.listForRepo({ owner, repo, state: 'open' }).catch(() => ({ data: [] })),
      octokit.repos.listLanguages({ owner, repo }).catch(() => ({ data: {} })),
      octokit.repos.listCommits({ owner, repo }).catch(() => ({ data: [] })),
    ...
    const metrics = {
      ...
      testCoverage: calculateMetric(Math.random(), 1), // Placeholder
